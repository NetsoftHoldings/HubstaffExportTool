...
class HubstaffExport
  ...
  def parsed_options?
    ...
    @opts_parser = OptionParser.new do |opts|
      ...
      opts.on('-o', '--organizations ORGANIZATIONS',
              'comma separated list of organization IDs (required)') do |organizations|
  ...
  def client_config
    fail 'Please use authenticat command first' unless File.exist?('hubstaff-client.cfg')
  ...
  def authenticate(app_token, email, password)
    ...
    File.open('hubstaff-client.cfg', 'w') do |file|
      file.write({auth_token: response['user']['auth_token'], app_token: app_token, email: email}.to_json)
    end
  ...
  def directory_for_screenshot(screenshot)
    File.join(@options.directory, "project - #{screenshot['project_id']}", "user - #{screenshot['user_id']}",
